...
/*   Updated: 2024/07/24 09:13:11 by hshimizu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cone_calculate_aabb(t_cone *self)
{
	...
	const t_aabb				aabb = {{{-1, 0, -1}}, {{1, 1, 1}}};

	figure->aabb = aabb_transform(&aabb, &figure->rotation, &figure->position);
}

t_vec3	cone_get_normal(t_cone *self, const t_vec3 *point)
{
	t_abstract_figure *const	figure = (void *)self;
	...
	return (vec3_norm(mat3x3_mul_vec3(mat3x3_transpose(figure->rotation_inv), \
		(t_vec3){{local_point._[0], 1 - local_point._[1], local_point._[2]}})));
}

void	cone_get_uv_coord(t_cone *self, const t_vec3 *point,
	float uv[2])
{
	t_abstract_figure *const	figure = (void *)self;
